...
public class CommentActivity extends Activity {
    private static final int column = 1;//3列
    private static final int pageCount = 1; //每页加载个数
    ...
    private void addImageView2Column() {
        ...
        //接收name值
        String urls= bundle.getString("name");
	...
	private void downloadImage(final ImageView itemImage, int index) {
      ...
      //接收name值
      String urls= bundle.getString("name");
      String content= bundle1.getString("name1");
        //System.out.println("获取到的name值为"+urls); 
    	//int c=urls.length;
    ...
    //  final String str=content[index].substring(0,10)+"\n"+content[index].substring(11,m)+"...";
        ...
        ImageRequest request = new ImageRequest(urls, new Listener<Bitmap>() {
            @Override//ImageConst.
            public void onResponse(Bitmap response) {
               // itemImage.setImageBitmap(response);
            	Drawable mDrawable =CommentActivity.this.getResources().getDrawable(R.drawable.fenf);
            	Bitmap mBitmap = ((BitmapDrawable) mDrawable).getBitmap();//////////////
               
                int width =response.getWidth(), hight =response.getHeight();
                System.out.println("宽"+width+"高"+hight);                               
                Bitmap icon = Bitmap.createBitmap(width, hight*6, Bitmap.Config.ARGB_8888); //建立一个空的BItMap  
