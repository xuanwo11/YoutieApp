...
public class NaviFragment extends Fragment implements OnClickListener
   ...
   // private TextView navi_intro;
    ...
    PersonFragment mPersonalFragment;/////////////////////////////
    ...
    private void OnTabSelected(int index) {
        ...
        case SETTINGSFRAGMENT: 
    		...
    		if(MyApplication.getInstance().getCurrentUser()==null){
    			ActivityUtil.show(mActivity, "请先登录。");
    			Intent intent1 = new Intent();
    			intent1.setClass(mActivity, RegisterAndLoginActivity.class);
    			MyApplication.getInstance().getTopActivity().startActivity(intent1);
    			return;
    		}else{
    	
    			Intent intent2 = new Intent();
    			intent2.setClass(MyApplication.getInstance().getTopActivity(), PersonActivity.class);
...
//			agent.startFeedbackActivity();
        	...
        	BmobUser currentUser = BmobUser.getCurrentUser(mActivity);//mActivity
        	
			if (currentUser != null) {
				// 允许用户使用应用,即有了用户的唯一标识符，可以作为发布内容的字段
				hideFragments(transaction);	//////////////////////////////////////////
				getActivity().getWindow().setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN, WindowManager.LayoutParams.FLAG_FULLSCREEN); //隐藏底部任务栏代码
	        	if(null == mFavFragment){
	        		...
	        		//ImageView img=(ImageView)getActivity().findViewById(R.id.item_action_fav);
	        		//img.setImageResource(R.drawable.ic_action_fav_choose);
	        	}
			} else {
				// 缓存用户对象为空时， 可打开用户注册界面…
