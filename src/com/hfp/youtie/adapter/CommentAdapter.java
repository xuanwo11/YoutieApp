...
public class CommentAdapter extends BaseContentAdapter<Comment>{
	...
	public View getConvertView(int position, View convertView, ViewGroup parent) {
		...
		if(MyApplication.getInstance().getCurrentUser()!=null){/////////////////
			...
			Log.i("bmob",id);
			String idd=comment.getUser().getObjectId();				
     ...
     viewHolder.userLogo.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(MyApplication.getInstance().getCurrentUser()==null){
					...
					MyApplication.getInstance().setCurrentComment(comment);
//				User currentUser = BmobUser.getCurrentUser(mContext,User.class);
//				if(currentUser != null){//已登录
					Intent intent = new Intent();
					intent.setClass(MyApplication.getInstance().getTopActivity(), PersoncomActivity.class);
